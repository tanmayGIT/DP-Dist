function [DistanceVal] = IsometricTransformDTW(Data_Ref,Data_Target,method)


% Implementatoin of the paper : 
% Górecki, T., & Luczak, M. (2014). Non-isometric transforms in time 
% series classification using DTW. Knowledge-Based Systems, 
% 61, 98–108. https://doi.org/10.1016/j.knosys.2014.02.011
...
%     D_refData = dst(Data_Ref);
      ...
      D_targetData = dstn(Data_Target);
      D_refData = dstn(Data_Ref);
      
elseif(strcmp(method,'cos'))
    D_targetData = dct(Data_Target);
    D_refData = dct(Data_Ref);

    D_targetData1 = dctn(Data_Target);
    D_refData1 = dctn(Data_Ref);
...
for ii = 1:1:size(signal,1)
    for jj = 1:1:size(signal,2)
        mySum = 0;
        for tt  = 1:1:size(signal,2)
            if( tt ~= jj)
                mySum = mySum + signal(ii,tt)/(jj-tt);
            end
        end
