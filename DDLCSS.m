function [DistanceVal,indxCol,indxRw] = DDLCSS(refSample,testSample)
% Assuming that the feature matrix are 2D/1D
% epsilon = 4*min( (std(mean(refSample,1))),(std(mean(testSample,1))) );
refSample = chotaDerivative(refSample);
refSample = chotaDerivative(refSample);

testSample = chotaDerivative(testSample);
testSample = chotaDerivative(testSample);
...
function D_targetData = chotaDerivative(Data_Target)
Data_Target = Data_Target';
[Target_Row, Target_Col] = size(Data_Target);

D_targetData = Data_Target;

%% Transformation with Derivative %%
for i=1:Target_Row
    for j=2:(Target_Col-1)-1
        D_point = Data_Target(i,j+1)-Data_Target(i,j);
        if j == 2
