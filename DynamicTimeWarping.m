function [DistanceVal] = DynamicTimeWarping(refSample,testSample) %#codegen
...
% "Dynamic Time Warping Algorithm Review" by Pavel Senin
    ...
    k = 1;
%     Wrapped = zeros((noOfSamplesInRefSample*noOfSamplesInTestSample),2); % This is the maximum no. of matching possible
%     Wrapped(end,:)=[X,Y];
%     cnt = (noOfSamplesInRefSample*noOfSamplesInTestSample);
    while ((X>1)||(Y>1))
        if ((X-1)==0)
            Y = Y-1;
        elseif ((Y-1)==0)
...
%  updatedWrapped = zeros(k,2);
%  updatedWrapped(1:k,:) = Wrapped((((noOfSamplesInRefSample*noOfSamplesInTestSample)-k)+1):end,:);     
%  indxRw =  updatedWrapped(:,1);%1:(size(refSample,1));
%  indxRw = indxRw';
%  indxCol = updatedWrapped(:,2);%1:(size(testSample,1));
%  indxCol = indxCol';
